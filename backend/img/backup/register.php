...
if (move_uploaded_file($_FILES['image']['tmp_name'], $imagePathOnServer)) {
    ...
    $imagePath = 'http://localhost/api_admin_dashboard/img/profile/' . $imageName;
} else {
...
try {
    $sql = "INSERT INTO employee (
                emp_ids, image_path, full_name, email, password, telephone, address, department, salary, status, start_work
            ) VALUES (
                :emp_ids, :image_path, :full_name, :email, :password, :telephone, :address, :department, :salary, :status, :start_work
            )";
